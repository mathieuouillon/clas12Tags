...
void hipo_output::recordSimConditions(outputContainer* output, map<string, string> sims) {
	...
	for (map<string, string>::iterator it = sims.begin(); it != sims.end(); it++) { if (it->first != "JSON") { data.push_back(it->first + ":  " + it->second + "  "); } }
	...
	string bigData;
	for (auto b : data) { bigData += b + string("\n"); }
...
int hipo_output::getDetectorID(string hitType) {
	if (detectorID.find(hitType) != detectorID.end()) { return detectorID[hitType]; }
	else {
...
string hipo_output::getHipoVariableName([[maybe_unused]] string trueInfoVar) {
	if (trueInfoNamesMap.find(trueInfoVar) != trueInfoNamesMap.end()) { return trueInfoNamesMap[trueInfoVar]; }
	else { return trueInfoVar; }
}
...
void hipo_output::writeHeader([[maybe_unused]] outputContainer*    output,
	...
	else if (fieldScales.find("binary_torus") != fieldScales.end()) { runConfigBank.putFloat("torus", 0, fieldScales["binary_torus"]); }
	else { runConfigBank.putFloat("torus", 0, 0); }
	if (fieldScales.find("clas12-newSolenoid") != fieldScales.end()) { runConfigBank.putFloat("solenoid", 0, fieldScales["clas12-newSolenoid"]); }
	else if (fieldScales.find("binary_solenoid") != fieldScales.end()) { runConfigBank.putFloat("solenoid", 0, fieldScales["binary_solenoid"]); }
	else { runConfigBank.putFloat("solenoid", 0, 0); }
	...
	if (rasterInitialized == -99) {
		...
		if (getenv("CCDB_CONNECTION") != nullptr) { connection = (string)getenv("CCDB_CONNECTION"); }
...
void hipo_output::writeUserInfoseHeader([[maybe_unused]] outputContainer*    output,
                                        [[maybe_unused]] map<string, double> data) {
	if (data.size() > 0) {
		...
		for (auto it = data.begin(); it != data.end(); it++) {
			...
			else { userLundBank.putFloat("userVar", index - lundStandardSize, (float)it->second); }
		}

		outEvent->addStructure(mcEventHeaderBank);
		if (userBankSize > 0) { outEvent->addStructure(userLundBank); }
	}
}


void hipo_output::writeRFSignal([[maybe_unused]] outputContainer*    output,
                                [[maybe_unused]] FrequencySyncSignal rfsignals,
                                [[maybe_unused]] gBank               bank) {
	int verbosity = int(output->gemcOpt.optMap["BANK_VERBOSITY"].arg);
	...
	if (verbosity > 2) { runRFBank.show(); }

	outEvent->addStructure(runRFBank);
}

void hipo_output::writeGenerated([[maybe_unused]] outputContainer*             output,
                                 [[maybe_unused]] vector<generatedParticle>    MGP,
                                 [[maybe_unused]] map<string, gBank>*          banksMap,
                                 [[maybe_unused]] vector<userInforForParticle> userInfo) {
	...
	if (verbosity > 2) { geantParticleBank.show(); }
...
void hipo_output::writeAncestors([[maybe_unused]] outputContainer*     output,
                                 [[maybe_unused]] vector<ancestorInfo> ainfo,
                                 [[maybe_unused]] gBank                bank) {
	vector<int>    pid;
	vector<int>    tid;
	vector<int>    mtid;
...
void hipo_output::initBank([[maybe_unused]] outputContainer* output,
                           [[maybe_unused]] gBank            thisHitBank,
                           [[maybe_unused]] int              what) {
}

void hipo_output::prepareEvent([[maybe_unused]] outputContainer*     output,
                               [[maybe_unused]] map<string, double>* configuration) {
	int verbosity             = int(output->gemcOpt.optMap["BANK_VERBOSITY"].arg);
	int nBankEntries          = 0;
	lastHipoTrueInfoBankIndex = 0;

	for (auto& conf : *configuration) {
		if (verbosity > 1) { cout << " Hipo preparing " << conf.first << "  with " << conf.second << " hits " << endl; }
		nBankEntries = nBankEntries + conf.second;
	}

	if (verbosity > 1) { cout << " Total true info bank entries: " << nBankEntries << endl; }
			...
			for (unsigned int nh = 0; nh < HO.size(); nh++) {
				...
				for (auto& thisVar : theseRaws) {
					...
					if (thisVar.first == bname) {
						...
						if (hipoName == "hitn") { thisVar.second = nh + 1; }

						if (varType == "i") { trueInfoBank->putInt(hipoName.c_str(), hipoBankIndex, thisVar.second); }
						else if (varType == "d") { trueInfoBank->putFloat(hipoName.c_str(), hipoBankIndex, thisVar.second); }

						if (verbosity > 2) {
							cout << " Hit Type: " << hitType << ", detector id: " << detectorID << ", hit index " << nh << ", bank hit index " << hipoBankIndex << ", name " <<
								bname << ", hname "
...
void hipo_output::writeG4DgtIntegrated(outputContainer* output, vector<hitOutput> HO, string hitType, map<string, gBank>* banksMap) {
	...
	for (auto& bankName : dgtBank.orderedNames) {
		...
		if (bankName.second.find("ADC_") != string::npos) { if (detectorADCSchema.getEntryName(0) != "empty") { hasADCBank = true; } }

		// flag TDC content if any variable has TDC_ prefix detectorTDCSchema schema exists
		if (bankName.second.find("TDC_") != string::npos) { if (detectorTDCSchema.getEntryName(0) != "empty") { hasTDCBank = true; } }

		// flag WF content if any variable has WF_ prefix detectorWFSchema schema exists
		if (bankName.second.find("WF_") != string::npos) { if (detectorWFSchema.getEntryName(0) != "empty") { hasWFBank = true; } }
	}

	if (verbosity > 2) {
		if (hasADCBank) { cout << hitType << " has ADC bank." << endl; }
		if (hasTDCBank) { cout << hitType << " has TDC bank." << endl; }
		if (hasWFBank) { cout << hitType << " has WF bank." << endl; }
	}
				...
				for (unsigned int nh = 0; nh < HO.size(); nh++) {
					...
					for (auto& thisVar : theseDgts) {
						...
						if (thisVar.first == bname) {
							...
							if (bname == "sector" || bname == "layer" || bname == "order") { detectorADCBank.putByte(bname.c_str(), nh, thisVar.second); }
							else if (bname == "component") { detectorADCBank.putShort(bname.c_str(), nh, thisVar.second); }
							else {
								// all other ADC vars must begin with "ADC_"
								if (bname.find("ADC_") == 0) {
									string adcName = bname.substr(4);
									if (varType == "i") { detectorADCBank.putInt(adcName.c_str(), nh, thisVar.second); }
									else if (varType == "d") { detectorADCBank.putFloat(adcName.c_str(), nh, thisVar.second); }
									else if (varType == "l") { detectorADCBank.putLong(adcName.c_str(), nh, thisVar.second); }
								}
							}
				...
				for (unsigned int nh = 0; nh < HO.size(); nh++) {
					...
					for (auto& thisVar : theseDgts) {
						...
						if (thisVar.first == bname) {
							...
							// sector, layer, component are common in adc/tdc so their names are w/o prefix
							// sector, layers are "Bytes"
							if (bname == "sector" || bname == "layer" || bname == "order") { detectorTDCBank.putByte(bname.c_str(), nh, thisVar.second); }
							else if (bname == "component") { detectorTDCBank.putShort(bname.c_str(), nh, thisVar.second); }
							else {
								// all other TDC vars must begin with "ADC_"
								if (bname.find("TDC_") == 0) {
									string adcName = bname.substr(4);
									if (varType == "i") { detectorTDCBank.putInt(adcName.c_str(), nh, thisVar.second); }
									else if (varType == "d") { detectorTDCBank.putFloat(adcName.c_str(), nh, thisVar.second); }
									else if (varType == "l") { detectorTDCBank.putLong(adcName.c_str(), nh, thisVar.second); }
								}
							}

							// do not repeat message logged above if hasADCBank was true
							if (verbosity > 2 && !hasADCBank) {
								cout << "hit index " << nh << ", name " << thisVar.first << ", value: " << thisVar.second << ", raw/dgt: " << bankType << ", type: " << varType <<
									endl;
							}
						}
					}
				...
				for (unsigned int nh = 0; nh < HO.size(); nh++) {
					...
					for (auto& thisVar : theseDgts) {
						...
						if (thisVar.first == bname) {
							...
							// sector, layer, component are common in adc/tdc so their names are w/o prefix
							// sector, layers are "Bytes"
							if (bname == "sector" || bname == "layer" || bname == "order") { detectorWFBank.putByte(bname.c_str(), nh, thisVar.second); }
							else if (bname == "component") { detectorWFBank.putShort(bname.c_str(), nh, thisVar.second); }
							else if (bname == "WF_timestamp") { detectorWFBank.putLong("timestamp", nh, thisVar.second); }
							else {
								// all other ADC vars must begin with "ADC_"
	...
	if (hasADCBank) {
		if (verbosity > 2) { detectorADCBank.show(); }
		outEvent->addStructure(detectorADCBank);
	}
	if (hasTDCBank) {
		if (verbosity > 2) { detectorTDCBank.show(); }
		outEvent->addStructure(detectorTDCBank);
	}
	if (hasWFBank) {
		if (verbosity > 2) { detectorWFBank.show(); }
...
void hipo_output::writeG4RawAll([[maybe_unused]] outputContainer*    output,
                                [[maybe_unused]] vector<hitOutput>   HO,
                                [[maybe_unused]] string              hitType,
                                [[maybe_unused]] map<string, gBank>* banksMap) {
}

void hipo_output::writeFADCMode1([[maybe_unused]] map<int, vector<hitOutput>> HO,
                                 [[maybe_unused]] int                         ev_number) {
}


void hipo_output::writeFADCMode1([[maybe_unused]] outputContainer*  output,
                                 [[maybe_unused]] vector<hitOutput> HO,
                                 [[maybe_unused]] int               ev_number) {
}

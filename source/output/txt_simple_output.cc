...
string indent(int depth) { return string(depth * INDENT_DEPTH, ' '); }
...
void txt_simple_output::writeRFSignal(outputContainer*       output, FrequencySyncSignal rfsignals,
	...
	*txtout << indent(1) << "RF Signals Bank {" << endl;
	...
	for (unsigned int i = 0; i < rfs.size(); i++) {
		...
		for (unsigned int j = 0; j < rfid.size(); j++) { *txtout << indent(2) << "RF " << i + 1 << " ID: " << rfid[j] << "   Signal: " << rfva[j] << endl; }
	}
	*txtout << indent(1) << "}" << endl;
}
...
void txt_simple_output::writeGenerated(outputContainer*    output, vector<generatedParticle>      MGP,
                                       map<string, gBank>* banksMap, vector<userInforForParticle> userInfo) {
	...
	for (unsigned int i = 0; i < MAXP && i < MGP.size(); i++) {
		...
		*txtout << indent(3) << "vert:\t" << MGP[i].vertex / mm << " mm" << endl;
		...
		if (MGP[i].fastMC.size() > 0) {
			if (MGP[i].pSum.size() != MGP[i].fastMC.size()) { cout << " !!! Warning: pSum and fastMC info do not match" << endl; }
			else { writeFastMC = 1; }
		}
	...
	if (userInfo.size()) {
		*txtout << indent(2) << "Generated User Info Particles Bank:" << endl;
		for (unsigned p = 0; p < userInfo.size(); p++) {
			*txtout << "      ";
			for (unsigned i = 0; i < userInfo[p].infos.size(); i++) { *txtout << userInfo[p].infos[i] << " "; }
			*txtout << indent(2) << endl;
		}
	}
...
void txt_simple_output::writeAncestors(outputContainer*       output, vector<ancestorInfo> ainfo,
                                       [[maybe_unused]] gBank bank) {
	ofstream* txtout = output->txtoutput;
...
void txt_simple_output::writeG4RawIntegrated(outputContainer* output, vector<hitOutput> HO, string hitType, map<string, gBank>* banksMap) {
	...
	ofstream* txtout = output->txtoutput;
	*txtout << indent(1) << thisHitBank.bankName << " (" << thisHitBank.idtag << ", " << DETECTOR_BANK_ID << ") integrated true infos bank (" << thisHitBank.idtag + RAWINT_ID <<
		", 0) {" << endl;
...
void txt_simple_output::writeG4RawAll(outputContainer* output, vector<hitOutput> HO, string hitType, map<string, gBank>* banksMap) {
	...
	ofstream* txtout = output->txtoutput;
	*txtout << indent(1) << thisHitBank.bankName << " (" << thisHitBank.idtag << ", " << DETECTOR_BANK_ID << ") step by step true infos bank (" << thisHitBank.idtag + RAWSTEP_ID <<
		", 0) {" << endl;
...
void txt_simple_output::writeG4DgtIntegrated(outputContainer* output, vector<hitOutput> HO, string hitType, map<string, gBank>* banksMap) {
	...
	ofstream* txtout = output->txtoutput;
	*txtout << indent(1) << thisHitBank.bankName << " (" << thisHitBank.idtag << ", " << DETECTOR_BANK_ID << ") integrated digitized bank (" << thisHitBank.idtag + DGTINT_ID <<
		", 0) {" << endl;
...
void txt_simple_output::writeChargeTime(outputContainer* output, vector<hitOutput> HO, string hitType, map<string, gBank>* banksMap) {
	...
	ofstream* txtout = output->txtoutput;
	*txtout << indent(1) << thisHitBank.bankName << " (" << thisHitBank.idtag << ", " << DETECTOR_BANK_ID << ") charge time infos (as seen by electronics) bank  (" << thisHitBank.
		idtag + CHARGE_TIME_ID << ", 0) {" << endl;
...
void txt_simple_output::writeFADCMode1([[maybe_unused]] outputContainer*  output,
                                       [[maybe_unused]] vector<hitOutput> HO,
                                       [[maybe_unused]] int               event_number) {
}


void txt_simple_output::writeFADCMode1([[maybe_unused]] map<int, vector<hitOutput>>,
                                       [[maybe_unused]] int) {
}

// write fadc mode 7 (integrated mode) - jlab hybrid banks. This uses the translation table to write the crate/slot/channel
void txt_simple_output::writeFADCMode7([[maybe_unused]] outputContainer*  output,
                                       [[maybe_unused]] vector<hitOutput> HO, [[maybe_unused]]
                                       int                                event_number) {
}
